...
export function ChatHeader() {
  ...
  return (
    <>
      {/* Mobile: fixed sources toggle — same level as AppSidebar hamburger */}
      <button
        onClick={() => setChatSidebarOpen(!chatSidebarOpen)}
        className="fixed top-3 left-[52px] z-40 md:hidden h-9 w-9 rounded-lg flex items-center justify-center bg-sidebar border border-sidebar-border text-sidebar-muted-foreground hover:text-sidebar-accent-foreground shadow-md"
        title="Toggle sources"
      >
        <PanelLeft className="h-4 w-4" />
      </button>

      <header className="flex items-center gap-2 pl-24 pr-3 md:px-5 py-2.5 border-b bg-card/80 backdrop-blur-sm flex-shrink-0">
