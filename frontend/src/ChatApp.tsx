...
import {
  LogOut, Upload, X, MessageSquare, FileText,
  Sparkles, Loader2, RotateCcw, Library, ArrowUp, ChevronRight,
} from 'lucide-react'
...
export default function ChatApp() {
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      {/* ── Sidebar ─────────────────────────────────────────────────── */}
      <aside className="w-72 flex-shrink-0 flex flex-col bg-zinc-950 border-r border-zinc-800/60">

        {/* Brand row */}
        <div className="flex items-center gap-2.5 px-4 py-3.5 border-b border-zinc-800/60">
          <div className="h-7 w-7 rounded-lg bg-gradient-to-br from-blue-500 to-violet-600 flex items-center justify-center flex-shrink-0 shadow-sm">
            <Library className="h-3.5 w-3.5 text-white" />
          </div>
          <span className="font-semibold text-white tracking-tight text-sm flex-1">RAG Q&amp;A</span>
          <ModeToggle className="h-7 w-7 text-zinc-500 hover:text-white hover:bg-zinc-800" />
          <Button
            variant="ghost" size="icon"
            onClick={logout}
            title="Log out"
            className="h-7 w-7 text-zinc-500 hover:text-red-400 hover:bg-zinc-800/80"
          >
        ...
        <div className="px-3 pt-3 pb-2">
          <button
            ...
            disabled={uploading}
            className={cn(
              'w-full flex items-center justify-center gap-2 rounded-lg border border-dashed border-zinc-800',
              'py-2.5 text-xs font-medium text-zinc-500 transition-all duration-200',
        ...
        <div className="px-4 pt-1 pb-1.5 flex items-center gap-2">
          ...
          {documents.length > 0 && (
            <span className="text-[10px] bg-zinc-800 text-zinc-500 rounded-full px-1.5 leading-5">
              {documents.length}
            </span>
          )}
        </div>
            ...
            documents.map(doc => (
              <button
                ...
                className={cn(
                  ...
                  selectedDoc?.doc_id === doc.doc_id
                    ? 'bg-blue-600/15 text-blue-300 ring-1 ring-inset ring-blue-500/25'
              ...
              >
                <FileText className={cn(
                  ...
                  selectedDoc?.doc_id === doc.doc_id ? 'text-blue-400' : 'text-zinc-700 group-hover:text-zinc-500'
                )} />
                <span className="truncate flex-1 text-xs font-medium">{doc.title ?? doc.filename}</span>
                <span
                  role="button"
                  aria-label="Delete"
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        {!selectedDoc ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-6 px-8 select-none">
            ...
            <p className="text-[11px] text-muted-foreground/50">or select an existing document in the sidebar</p>
          </div>
        ) : (
          <>
            {/* Header */}
