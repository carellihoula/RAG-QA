  ...
  async function handleKbFilesChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (const file of files) {
      try {
        ...
        uploaded++;
        toast.loading(`Uploading ${uploaded} / ${files.length} PDFs…`, {
          id: toastId,
        });
      } catch (err) {
    ...
    await fetchKbDocs(kbId);
    toast.success(
      `${uploaded} PDF${uploaded !== 1 ? "s" : ""} added to "${kbName}"`,
      { id: toastId },
    );
        ...
        <aside className="w-64 flex-shrink-0 flex flex-col bg-sidebar border-r border-sidebar-border overflow-hidden">
              ...
              documents.map((doc) => {
                ...
                return (
                  ...
                  >
                    {(() => {
                      const sm =
                        SOURCE_ICONS[doc.source_type ?? "pdf"] ??
                        SOURCE_ICONS.pdf;
                      return (
                        <sm.icon
                          className={cn(
          ...
          <div className="px-3 py-3 border-t border-sidebar-border flex-shrink-0">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-[10px] text-sidebar-muted-foreground/50">
                {knowledgeBases.length}/{MAX_KBS} KB · {documents.length}/
                {MAX_DOCS} docs
              </span>
            </div>
            <div className="flex flex-col gap-0.5">
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-6 px-8 select-none">
              ...
              <div className="text-center space-y-2 max-w-sm">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">
                  Select a <strong>Knowledge Base</strong> to query multiple
                  documents at once, or pick a single document from the sidebar.
                </p>
              </div>
                    ...
                    <>
                      ...
                      >
                        <Library
                          className={cn(
                            "h-3 w-3",
                            KB_COLORS[selectedKb.color]?.text ??
                              "text-violet-500",
                          )}
                        />
                      </div>
